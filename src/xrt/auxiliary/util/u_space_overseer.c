...
create_local_space(struct xrt_space_overseer *xso, struct xrt_space **out_space)
{
	...
	xrt_device_get_tracked_pose(xso->headdev, XRT_INPUT_GENERIC_HEAD_POSE, timestamp_ns, &xsr);
...
destroy(struct xrt_space_overseer *xso)
{
	...
	for (int id = 0; id < IPC_MAX_LOCAL_SPACES; id++) {
		struct xrt_space **xslocal_ptr = (struct xrt_space **)&xso->xrtlocalspace[id];
