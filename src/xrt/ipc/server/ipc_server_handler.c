...
get_new_localspace_id(volatile struct ipc_client_state *ics, uint32_t *out_id)
{
	...
	uint32_t index = 0;
	for (; index < IPC_MAX_CLIENT_SPACES; index++) {
		if (ics->server->xso->xrtlocalspace[index] == NULL) {
			break;
		}
	}
...
create_localspace(volatile struct ipc_client_state *ics, uint32_t *out_local_id)
{
	...
	xrt_space_overseer_create_local_space(xso, &xso->xrtlocalspace[ics->local_space_overseer_index]);

	xrt_space_reference(xs_ptr, xso->xrtlocalspace[ics->local_space_overseer_index]);
	*out_local_id = id;
...
ipc_handle_space_destroy(volatile struct ipc_client_state *ics, uint32_t space_id)
{
	...
	if (space_id == ics->local_space_index) {
		struct xrt_space **xslocal_ptr =
		    (struct xrt_space **)&ics->server->xso->xrtlocalspace[ics->local_space_overseer_index];
